	...
	$.widget("wgm.embed_imgNotes", {
		options: {
			...
			onShow: function(ev, elem) {
				...
				return $('<div id="NoteDialog"></div>').dialog({
					...
					width: 250,
					minHeight:"auto",
					position: { my: "top+31", at: "bottom", of: elem, within: '#embed_pod_image', collision: "flipfit"},
					buttons: {
						"" : function() {
							$(this).dialog("close");
						}
		...
		_create: function() {
			...
			if (!this.element.is("img")) {
				$.error('embed_imgNotes plugin can only be applied to img elements');
			}
		...
		embed_hide_show_hotspots: function() {
			var self = this;
			var total = self.notes.length;
			for ( var i = 0; i < total; i++ ){
				var $this = self.notes[i];
		...
		import: function(notes) {
			var self = this;
			$.each(notes, function() {
				self.addNote(this.x, this.y, this.note, this.hs_color), this.circle;
			});
			$(this.img).imgViewer("update");
		},
