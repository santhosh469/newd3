    ...
    $.widget("wgm.imgNotes", {
        options: {
            ...
            onShow: function(ev, elem) {
                var $elem = $(elem);
                $('.viewport, .rny-fullviewimg-container').on('mousedown', function(ev) {
                    if ($("#hs_media_container").show() && !$.contains($("#hs_media_container")[0], ev.target)) {
                        $("#hs_media_container").hide();
                        ev.preventDefault();
                    }
                });

                $('#hs_media_container').remove();
                $($elem).append('<div id="hs_media_container">' + $elem.data("note") + '</div>');
                return $('#hs_media_container').position({ my: "top+30", at: "bottom", of: elem, within: '#pod_image', collision: "flipfit" });
            },
