 ...
 */			
			self.zimg = $("<img/>", {"src": self.img.src}).appendTo(".rny-fullviewimg-container").wrap("<div class='viewport'/>");
			var $zimg = $(self.zimg);
 ...
 */
		_bind_zoom_events: function() {
			...
			// $(document).on('click', '#hsZoom', function (event) {
   ...
   //          });
            ...
            $("#normal_image").on("click", function(event){
                event.preventDefault();
                var delta = 0;
                self.options.zoom = 0;
                self.update();
 ...
 */
				...
				$(this.zimg).css({transform: "translate(" + xt + "px," + yt + "px) scale(" + zoom + "," + zoom + ")",transition: "transform 1s", "-webkit-transition":"transform 1s","-moz-transition":"transform 1s" });
/*
