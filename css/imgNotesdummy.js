...
;(function($) {
	$.widget("wgm.imgNotes", {
		options: {
			...
			// 	return  $('<div class="black"><div class="hs-label-tag"></div><div class="hs-label">hotspot label</div></div>');
			// },
			...
			onShow: function(ev, elem) {
				...
				// var $dlgClose = $('.viewport, .rny-fullviewimg-container');
				// $("<div id='NoteDialog'><div id='NoteDialog1'></div></div>");
				$('#note1').remove();
				$($elem).append("<div id='note1'>"+$elem.data("note")+"</div>");
				return $('#NoteDialog').position({my: "top+31", at: "bottom", of: elem, within: '#pod_image', collision: "flipfit"});
		// .dialog({
		// 			modal: false,
		// 			resizable: false,
		// 			height: "auto",
		// 			width: "auto",
		// 			minHeight:"auto",
		// 			position: { my: "top+31", at: "bottom", of: elem, within: '#pod_image', collision: "flipfit"},
		// 	open: function() {
		// 		//Get the note text and put it into the textarea for editing
  ...
  //           	var dlg = this;
  //           	var $dlgClose = $('.viewport, .rny-fullviewimg-container');
  //           	$dlgClose.on('mousedown.mydialog', function(ev) {
  ...
  //       	},
	 //        close: function() {
	 //            $dlgClose.off('mousedown.mydialog');
	 //            $(this).dialog("destroy");
	 //        }
		// });
				// $(".hs-label").position({
    ...
    // 				collision: "flipfit"
		...
		addNote: function(relx, rely, text, elem) {
			var self = this;
			this.noteCount++;
			var $elem = $(elem);
			$(this.img).imgViewer("addElem",elem);
		...
		hide_show_hotspots: function() {
			...
			for ( var i = 0; i < total; i++ ){
				...
				if ($this.css("display") == 'block') {
					...
					// console.log(i);
					// self.options.onShow.call(self, this).remove();
					$("#NoteDialog").hide();
				}
				else {
		...
		import: function(notes) {
			var self = this;
			$.each(notes, function() {
				self.addNote(this.x, this.y, this.note, this.hs_color);
			});
			$(this.img).imgViewer("update");
		},
